      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
          {stats.filter((stat) => {
            ...
            if (!isAdmin) {
              return ['Total Events', 'Published Events', 'Total Registrations', 'Total Attendees'].includes(stat.name)
            }
            return true
          }).map((stat) => {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
            <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">Events by Status</h3>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
            <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">Registration Overview</h3>
            <ResponsiveContainer width="100%" height={300}>
              <BarChart
                data={[
                  { name: 'Total', value: analytics?.totalRegistrations || 0 },
                  { name: 'Attendees', value: analytics?.totalAttendees || 0 },
                  { name: 'Pending', value: (analytics?.totalRegistrations || 0) - (analytics?.totalAttendees || 0) },
                ]}
              >
        ...
        {isAdmin && (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="bg-gradient-to-br from-blue-500 to-blue-600 rounded-xl shadow-lg p-6 text-white">
              <h3 className="text-sm font-medium opacity-90 mb-2">Event Performance</h3>
              <p className="text-3xl font-bold">
                {analytics?.totalEvents > 0
                  ? ((analytics?.publishedEvents / analytics?.totalEvents) * 100).toFixed(0)
                  : 0}%
              </p>
            ...
            <div className="bg-gradient-to-br from-purple-500 to-purple-600 rounded-xl shadow-lg p-6 text-white">
              ...
              <p className="text-3xl font-bold">
                {analytics?.totalEvents > 0
                  ? Math.round((analytics?.totalAttendees || 0) / analytics?.totalEvents)
                  : 0}
              </p>
            ...
            <div className="bg-gradient-to-br from-emerald-500 to-emerald-600 rounded-xl shadow-lg p-6 text-white">
              <h3 className="text-sm font-medium opacity-90 mb-2">Revenue per Event</h3>
              <p className="text-3xl font-bold">
                ${analytics?.totalEvents > 0
                  ? (parseFloat(analytics?.totalRevenue || 0) / analytics?.totalEvents).toFixed(2)
                  : '0.00'}
              </p>
        ...
        {!isAdmin && (
          <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4 mt-6">
            <p className="text-sm text-blue-800 dark:text-blue-200">
              <strong>Limited Access:</strong> You have view-only access to events and registrations. 
              Contact your tenant administrator for full access or to make changes.
            </p>
          </div>
        )}
      </div>
